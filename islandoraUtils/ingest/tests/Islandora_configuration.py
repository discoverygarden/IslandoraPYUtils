...
class TestIslandora_configuration(unittest.TestCase):
    ...
    def test_configuration_dictionary(self):
        expected_result = {'Drupal': {'url': 'http://localhost/drupal'}, 'Solr': {'url': 'http://localhost:8080/solr'}, 'Fedora': {'url': 'http://localhost:8080/fedora', 'username': 'fedoraAdmin', 'password': 'islandora'}}
        self.assertEqual(expected_result, self.islandora_config.configuration_dictionary)
        
    @classmethod
    def setUp(self):
        #make a logg file in cwd
